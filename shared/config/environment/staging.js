// shared/config/environment/staging.js
/**
 * @file Staging Environment Configuration
 * @description Staging-specific settings and feature flags
 * @version 3.0.0
 */
...
class StagingConfig {
  constructor() {
    ...
    this.deployment = {
      ...
      branch: process.env.GIT_BRANCH || 'staging',
    ...
    // External service endpoints (staging)
    this.external = {
      sentry: {
      ...
      cloudinary: {
        cloudName: process.env.CLOUDINARY_CLOUD_NAME || 'staging-insightserenity',
        apiKey: process.env.CLOUDINARY_API_KEY,
        apiSecret: process.env.CLOUDINARY_API_SECRET
      },
      googleAnalytics: {
        trackingId: process.env.GA_TRACKING_ID,
        enabled: false // Disabled in staging to avoid polluting production analytics
      },
      intercom: {
    ...
    this.maintenance = {
      ...
      message: process.env.MAINTENANCE_MESSAGE || 'Staging environment is temporarily under maintenance for testing. Please check back soon.',
  ...
  getServiceUrl(envVar, defaultPath) {
    const baseUrl = process.env[envVar] || process.env.APP_URL || 'https://staging.insightserenity.com';
  ...
  validate() {
    ...
    if (missing.length > 0) {
      throw new Error(`Missing required environment variables for staging: ${missing.join(', ')}`);
    }
